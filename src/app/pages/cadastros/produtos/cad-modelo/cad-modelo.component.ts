...
@Component({
  selector: 'app-cad-modelo',
  templateUrl: './cad-modelo.component.html',
  styleUrls: ['./cad-modelo.component.css'],
  animations: [
...
export class CadModeloComponent implements OnInit {
  private configFuncoesBasicas = {
    tipo_ser_salvo: this.CONSTS.PRODUTOS.MODELO,
    rota: 'pages/listagems/produtos/listmodelo'
  }
    ...
    this.activeRouter.params.subscribe(
      res => {
        if (res.id != undefined) {
          ...
          this.readDados.carregaDadosById(this.configFuncoesBasicas.tipo_ser_salvo, res.id).subscribe(data => { this.nomeItem = data.nome; });
        } else {
  ...
  private verificacoes(): boolean {
    ...
    if (this.nomeItem == "") {
      this.avisoErro.createDialog(
        ...
        this.CONSTS.AVISO.MSGS.AVISO_CAMPO_NOME_CAD_MODELO_VASIO,
      );
      retorno = false;
      ...
      this.avisoErro.createDialog(
        ...
        this.CONSTS.AVISO.SIZES.SIZE_M.HEIGHT,
        this.CONSTS.AVISO.TIPOERROAVISO.AVISO,
        this.CONSTS.AVISO.MSGS.AVISO_CAMPO_NOME_CAD_MODELO_MENOR3,
      );
      retorno = false;
    } else {
      retorno = true;
